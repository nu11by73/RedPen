...
class DomainSubdomainEnumerator:
    ...
    def _vt_subdomains(self, domain):
        ...
        try:
            resp = requests.get(f"https://www.virustotal.com/api/v3/domains/{domain}/subdomains?limit=40", headers={"x-apikey": api_key}, timeout=15)
            if resp.status_code == 200:
    ...
    def _bruteforce(self, domain):
        ...
        common = ["www","mail","remote","blog","webmail","server","ns1","ns2","smtp","secure","vpn","m","shop","ftp","test","portal","support","dev","web","admin","store","cdn","api","app","staging","demo","beta","sandbox","internal","intranet","git","gitlab","jenkins","ci","jira","confluence","wiki","docs","status","monitor","grafana","kibana","docker","k8s","registry","backup","db","database","mysql","redis","qa","uat","preprod","prod","legacy","old","temp"]
        for sub in common:
            full = f"{sub}.{domain}"
            try:
    ...
    def _whois(self, domain):
        ...
        try:
            ...
            self.results["whois_info"] = {"registrar": str(w.registrar or "Unknown"), "creation_date": str(w.creation_date), "expiration_date": str(w.expiration_date), "name_servers": w.name_servers or []}
    ...
    def _shadow_it_subs(self):
        shadow_kw = ["dev","test","staging","temp","tmp","demo","sandbox","lab","poc","trial","beta","alpha","personal","my","old","legacy","backup","debug"]
        for sub_data in self.results["subdomains"]:
            sub = sub_data["subdomain"]
            for kw in shadow_kw:
                if kw in sub.split(".")[0]:
                    self.results["shadow_it_flags"].append({"type": "Suspicious Subdomain", "asset": sub, "reason": f"Contains '{kw}', suggesting dev/test/personal environment outside IT management."})
                    break
