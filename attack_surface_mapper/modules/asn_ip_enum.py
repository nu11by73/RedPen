...
class ASNIPEnumerator:
    ...
    def _bgp_asn_lookup(self, domain, company_name=None):
        console.print("[yellow][*] Querying ASN databases (RIPE Stat)...[/yellow]")
        # Use IPs we already resolved to find ASNs and their prefixes
        for ip_entry in self.results["ip_addresses"]:
            ip = ip_entry["ip"]
            try:
                # Get ASN for this IP via RIPE Stat
                url = f"https://stat.ripe.net/data/prefix-overview/data.json?resource={ip}"
                resp = requests.get(url, timeout=self.config["REQUEST_TIMEOUT"])
                if resp.status_code == 200:
                    data = resp.json().get("data", {})
                    for asn_entry in data.get("asns", []):
                        asn_num = str(asn_entry.get("asn", ""))
                        asn_holder = asn_entry.get("holder", "Unknown")
                        existing = [a["asn"] for a in self.results["asn_info"]]
                        if asn_num and asn_num not in existing:
                            self.results["asn_info"].append({
                                "asn": asn_num,
                                "description": asn_holder,
                                "source": "RIPE Stat"
                            })
                            console.print(f"  [green][+] RIPE: ASN{asn_num} - {asn_holder}[/green]")
                            # Now get all prefixes for this ASN
                            try:
                                prefix_url = f"https://stat.ripe.net/data/announced-prefixes/data.json?resource=AS{asn_num}"
                                prefix_resp = requests.get(prefix_url, timeout=self.config["REQUEST_TIMEOUT"])
                                if prefix_resp.status_code == 200:
                                    prefixes = prefix_resp.json().get("data", {}).get("prefixes", [])
                                ...
                                console.print(f"  [red][-] Prefix lookup failed for ASN{asn_num}: {e}[/red]")
                time.sleep(self.config["REQUEST_DELAY"])
            except Exception as e:
                console.print(f"  [red][-] RIPE Stat failed for {ip}: {e}[/red]")

        # Also try searching by company name if provided
        if company_name:
            try:
                url = f"https://stat.ripe.net/data/searchcomplete/data.json?resource={requests.utils.quote(company_name)}"
                resp = requests.get(url, timeout=self.config["REQUEST_TIMEOUT"])
                if resp.status_code == 200:
                    ...
                    for cat in categories:
                        if cat.get("category") == "ASNs":
                            for suggestion in cat.get("suggestions", []):
                                asn_num = str(suggestion.get("value", "")).replace("AS", "")
                                asn_desc = suggestion.get("label", "Unknown")
                                existing = [a["asn"] for a in self.results["asn_info"]]
                                if asn_num and asn_num not in existing:
                                    self.results["asn_info"].append({
                                        "asn": asn_num,
                                    ...
                                    console.print(f"  [green][+] RIPE Search: ASN{asn_num} - {asn_desc}[/green]")
            except Exception as e:
                console.print(f"  [red][-] RIPE company search failed: {e}[/red]")
